# -*- coding: utf-8 -*-
"""MLP desde cero.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1fi1396TJjfwEOZ7B112KIHNxkX9wJQ1g
"""
...
class  Violky(object):
  ...
  def derivas(self,y,y_hat):
    if self.perdida=='MSE': error=(2*(y-y_hat))/len(str(y_hat))  
    elif self.perdida=='RMSE': error=1/(2*np.sqrt((2*abs((y-y_hat)))/len(str(y_hat)))) 
    return error 
  ...
  def MLP(self, x_entrenamiento, y_entrenamiento, x_validacion, y_validacion, epocas, tasa_aprendizaje=0.1,eps=1e-8,beta1=0.9,beta2=0.999, alp=0.9):
    perdida_entre = []
    metrica_entre = []
    perdida_val = []
    metrica_val = []

    n_e=len(y_entrenamiento)
    n_v=len(y_validacion)
    ...
    for i in range(epocas):
      ...
      for j, entradas in enumerate(x_entrenamiento):   
        entradas_val = y_entrenamiento[j]
        ...
        p_e += self.l_met(entradas_val,y_hat)
        acc_e += self.met(y_hat,entradas_val)

      perdida_entre.append(p_e/n_e)
      metrica_entre.append(acc_e/n_e)

      if x_val.any():
        y_val_hat= self.propagacion_adelante(x_val)
        p_v = self.l_met(y_val, y_val_hat)
        acc_v = self.met(y_val_hat,y_val)

      perdida_val.append(p_v/n_v)
      metrica_val.append(acc_v/n_v)

      print("Epoca: {}, Per_Entren: {}, Prec_Entren: {}, Per_Val: {}, Prec_Val: {}".format(i+1,p_e/n_e,acc_e/n_e,p_v/n_v,acc_v/n_v))

      historico=pd.DataFrame({'Perdida Entrenamiento': perdida_entre, 'Perdida Validación': perdida_val, 'Metrica Entrenamiento': metrica_entre, 'Metrica Validación': metrica_val})
    return historico
...
# crear un perceptrón multicapa con tres capas ocultas
mlp = Violky(8, [6,4,2], 1, activacion='tanh', metrica='RMSE', optimizador='Adagrad', perdida='MSE')
...
epocas=62
# entrenar la red
hist=mlp.MLP(x_entre, y_entre,x_val,y_val, epocas)
...
inverted = escalar.inverse_transform(compara.valores)
...
plt.ylabel('RMSE')
plt.xlim(0,len(results))
plt.savefig("RMSE-MLP-Violky.jpg")
plt.show()
...
resul=[]
for i in range(31): 
  parcial=rnn.propagacion_adelante(x_test)
...
prediccionAgosto.to_csv('pronostico-MLP-WTI-Violky-SinLotes.csv')